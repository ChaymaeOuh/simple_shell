...
list_f *my_add_node_start(list_f **head, const char *str, int num)
{
	...
	if (!head)
		return (NULL);
	new_head = malloc(sizeof(list_f));
	if (!new_head)
		return (NULL);
	my_memset((void *)new_head, 0, sizeof(list_f));
	new_head->num = num;
	if (str)
	{
...
{
	list_f *new_node, *node;

	if (!head)
		return (NULL);

	node = *head;
	new_node = malloc(sizeof(list_f));
	if (!new_node)
		return (NULL);
	my_memset((void *)new_node, 0, sizeof(list_f));
	new_node->num = num;
	if (str)
	{
 ...
 * Return: size of list
 */
size_t my_print_list_str(const list_f *h)
{
	size_t i = 0;
 ...
 */
int my_delete_node_at_index(list_f **head, unsigned int index)
{
	list_f *node, *prev_node;
	unsigned int i = 0;
