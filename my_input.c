 ...
 * Return: bytes read
 */
ssize_t my_input_buf(info_f *info, char **buf, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
 ...
 * Return: bytes read
 */
ssize_t my_get_input(info_f *info)
{
 ...
 * Return: r
 */
ssize_t my_read_buf(info_f *info, char *buf, size_t *i)
{
	ssize_t r = 0;
 ...
 * Return: s
 */
int my_getline(info_f *info, char **ptr, size_t *length)
{
...
/**
 * my_sigint - blocks ctrl-C
 * @sig_num: the signal number
 *
 * Return: void
 */
void my_sigint(__attribute__((unused)) int sig_num)
{
	my_puts("\n");
	my_puts("$ ");
	my_putchar(NEG_ONE);
